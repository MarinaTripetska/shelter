export const body = document.querySelector("body");
export const mobileMenu = document.querySelector(".mobile-menu");
export const burger = document.getElementById("burger");
export const nav = document.getElementById("navigation");
export const navItemsColection = nav.querySelectorAll("a");
export const logo = document.querySelector(".logo");
//
export const modalOverlay = document.querySelector(".overlay");
export const modalBody = modalOverlay.querySelector(".modal");
export const closeModalBtn = modalOverlay.querySelector(".btn-close-modal ");
//
export const petCards = document.querySelectorAll(".card");
